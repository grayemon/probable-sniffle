...
class Settings(BaseSettings):
    ...
    chatwoot_agent_bot_api_key: str = Field(
        ..., description="Chatwoot Agent Bot API key"
    )
    chatwoot_user_api_key: str = Field(..., description="Chatwoot User API key")
    chatwoot_base_url: str = Field(
        default="http://localhost:3000", description="Chatwoot Base URL"
    )

    # Letta Configuration
    letta_api_key: str = Field(..., description="Letta API key")
    letta_base_url: str = Field(
        default="http://localhost:8283", description="Letta Base URL"
    )
    letta_agent_id: str = Field(..., description="Letta Agent ID")
